...
TEST(HumanSolve_test, findNakedSingle) {
    single_digit_human_solve_t test_table[] = {
        {(1 << 7), {8, 3, 2, &Play::insert}, 0, true, "200150074001000020407600013040200095070480060010000430706000289004907350003000040"},
        {(1 << 5), {6, 5, 6, &Play::insert}, 0, true, "412736589000000106568010370000050210100000008087090000030070865800000000000908401"},
    };
...
TEST(HumanSolve_test, findHiddenSingle) {
    single_digit_human_solve_t test_table[] = {
        {(1 << 0), {1, 6, 2, &Play::insert}, 0, true, "009032000000700000162000000010020560000900000050000107000000403026009000005870000"},
    };
...
TEST(HumanSolve_test, findXwing) {
    single_digit_human_solve_t test_table[] = {
        {(1 << 4), {5, 3, 4, &Play::pencil}, 1, true, "041729030769003402032640719403900170607004903195370024214567398376090541958431267"},
        {(1 << 0), {1, 1, 3, &Play::pencil}, 9, true, "980062753065003000327050006790030500050009000832045009673591428249087005518020007"},
    };
...
TEST(HumanSolve_test, findUniqueRectangle) {
    single_digit_human_solve_t test_table[] = {
        {((1 << 7) | (1 << 8)), {8, 1, 1, &Play::pencil}, 2, true, "502008967100700452067500381213657849654891273700004615821900034306000098005083026"},
    };
...
TEST(HumanSolve_test, findChainOfPairs) {
    single_digit_human_solve_t test_table[] = {
        {((1 << 3) | (1 << 4)), {5, 5, 6, &Play::pencil}, 1, true, "798452316603781092012030870370265048820143760060897023980014237107028050200070081"},
        {((1 << 3) | (1 << 0)), {0, 0, 0, NULL}, 0, false, "798452316603781092012030870370265048820143760060897023980014237107028050200070081"},
        {((1 << 1) | (1 << 6)), {7, 4, 8, &Play::pencil}, 1, true, "360859004519472386408613950146738295900541000005926401054387009093164500001295043"},
    };
